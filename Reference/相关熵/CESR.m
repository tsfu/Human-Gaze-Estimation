...
%             xF = pinv(wt1'*wt1)*(AF'*(weight.*b)-1/2*alpha*sum(length(F)));%  就是 alpha
        xF = pinv(wt1'*wt1)*(AF'*(weight.*b)-1/2*alpha*sum(W1));%  就是 alpha
            disp('warning by heran');
        else
%             xF = (wt1'*wt1)\(AF'*(weight.*b)-1/2*alpha*sum(length(F)));
       xF = (wt1'*wt1)\(AF'*(weight.*b)-1/2*alpha*sum(W1));
        end
    ...
    yG = wt1' * (AF * xF - b)+1/2*alpha*sum(W1); % alpha
    y(G) = yG;
